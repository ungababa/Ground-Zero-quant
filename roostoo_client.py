...
class RoostooClient:
    ...
    def _encode(self, params: dict[str, Any]) -> str:
        return '&'.join(["{}={}".format(k, params[k]) for k in sorted(params.keys())])
    ...
    def _get(
        ...
        log.debug("API REQUEST   GET %s  signed=%s  params=%s", path, signed, json.dumps(payload, default=str))
    ...
    def _post(
        ...
        log.debug("API REQUEST   POST %s  signed=%s  body=%s", path, signed, encoded_body)
